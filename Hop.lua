...
local FOLDER="ServerHop";local TIME=FOLDER.."/time.json"; local used = used or {}; local rng=Random.new(tick()*1e6%1e9);
...
local function GetRandom()local f=GetFiles()if #f==0 then return end;local d=Load(f[rng:NextInteger(1,#f)])if #d==0 then return end;for i=#d,2,-1 do local j=rng:NextInteger(1,i)d[i],d[j]=d[j],d[i]end;for i=1,#d do local id=d[i]if not used[id]then used[id]=true return id end end;used={}return d[rng:NextInteger(1,#d)]end
