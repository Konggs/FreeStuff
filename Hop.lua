...
while task.wait() do
    ...
    if s then
        local sid, players, ping = tostring(s.id), tonumber(s.playing), tonumber(s.ping)
        local ageMin = math.floor((s.age or 0)/60)
        print(("[AutoHopServer] Attempting server %s | Players: %d | Ping: %dms | Age: %dm")
            :format(sid, players, ping, ageMin))
        local ok, err = pcall(function()
            TeleportService:TeleportToPlaceInstance(placeId, sid, Player)
        end)
        if not ok then
            fails+=1
